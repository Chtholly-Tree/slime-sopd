...
from examples.logits_level_gold.sglang_api_request import _GOLD_METADATA_KEY, build_teacher_result_with_topk
...
async def generate(args: Any, sample: Sample, sampling_params, evaluation: bool = False) -> Sample:
    ...
    if not evaluation:
        from slime.rollout.multimodal_gold import _build_teacher_prompt_ids

        prepare_teacher_prompt_task = asyncio.create_task(asyncio.to_thread(_build_teacher_prompt_ids, args, sample))
    ...
    teacher_result = await build_teacher_result_with_topk(args, sample, teacher_prompt_ids)

    sample.metadata = sample.metadata or {}
    sample.metadata[_GOLD_METADATA_KEY] = {
