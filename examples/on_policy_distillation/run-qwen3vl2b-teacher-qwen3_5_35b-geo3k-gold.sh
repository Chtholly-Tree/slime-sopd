...
# Qwen3-VL GOLD-advantage training script adapted from run-qwen3-vl-opd.sh.
# Usage:
#   bash examples/on_policy_distillation/run-qwen3-vl-gold.sh
...
# Teacher server (SGLang, used only for GOLD teacher logprobs + decoded triplets)
...
SAVE_PATH=${SLIME_SCRIPT_SAVE_PATH:-"/mnt/tidal-alsh01/dataset/redone/zengyu/xikun/slime-2.4/checkpoints/qwen3-vl-2B_teacher_qwen3.5-35B_geo3k_2.4"}
...
GOLD_ARGS=(
   ...
   # no task reward participates in training; only GOLD-projected OPD signal is used
   --use-opd
   --opd-kl-coef 1.0
   --opd-type sglang
...
#    --use-wandb
   ...
   --wandb-group qwen3-vl-2B_teacher_qwen3.5-35B_geo3k_2.4
