...
def _get_teacher_tokenizer(args):
    ...
    return _get_tokenizer(teacher_hf_checkpoint)


def _build_teacher_prompt_text(sample: Sample) -> str:
...
def _build_teacher_token_ids(args, sample: Sample) -> tuple[list[int], int]:
    ...
    prompt_text = _build_teacher_prompt_text(sample)
...
def _to_canonical_pieces_from_ids(tokenizer, token_ids: list[int]) -> list[str]:
    ...
    for k in range(len(token_ids)):
        cur = tokenizer.decode(token_ids[: k + 1], skip_special_tokens=False, clean_up_tokenization_spaces=False)
...
def _prepare_gold_teacher_log_probs(args, sample: Sample, teacher_result: dict) -> torch.Tensor:
...
def post_process_rewards(args, samples: list[Sample], **kwargs):
    ...
    async def _gather():
        return await _asyncio.gather(*[_call_teacher(args, sample) for sample in samples])
    ...
    for sample, teacher_result, raw_reward in zip(samples, teacher_results, raw_scores, strict=False):
        sample.teacher_log_probs = _prepare_gold_teacher_log_probs(args, sample, teacher_result)
